...
and cores. Modern computers (@fig-cpu) have multiple CPUs, and those can have
...
in one core to perform the operation (@fig-sum).  
...
parallelized and use the CPU cores available in your system (@lst-matrix)
...
^[Also known as *perfectly parallel*, *delightfully parallel* or
...
processes that run in parallel, saving time and effort (@fig-daworflowpar).
...
One example can be bootstrapping model coefficients (@lst-loop). In this
...
logic of the `for` loop in a function (@lst-lapply):
...
we will use `map` function in the `purrr` package (@lst-purrr):
...
exactly the same as lapply (@lst-mclapply):
...
reuse the `purrr` example, but using `in_parallel` (@lst-mirai):

```{r}
#| lst-label: lst-mirai
#| lst-cap: "Boostrapping model coefficients in iris dataset with in_parallel"
#| warning: false
...
scenarios:
  ...
  1. We are parallelizing processes that involve sending a lot of data to each
  ...
  1. Related to the previous, when performing memory intensive processes. Take
